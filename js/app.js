// ── Main app logic, boot, event wiring ──
...
import { toast, addLog, setSt, openPanel, closePanel, openDebrief as _openDebrief, closeDebrief, setBg, togNotif, togPenalty, saveApiKey } from './ui.js';
import { renderAll, updClock, flashT, xpFloat, getLvl, getLvlInfo, msUntil } from './render.js';
...
// Wrap openDebrief to sync stat counters when panel opens
...
function markDone(id) {
  ...
  renderAll();
  flashT(id, 'g');
  xpFloat(xpGain, document.getElementById('ti-'+id));
  fireN(`✅ DONE: ${task.label}`, `+${xpGain} XP${wasFail?' (partial — task was failed)':'. Keep going.'}`, 'complete');
...
function logFunAct(id) {
  ...
  save(state.data);
  renderAll();
  const el = document.querySelector(`[onclick="window.logFunAct('${id}')"]`);
  xpFloat(act.xp, el);
  const msg = act.xp > 0
    ? `${act.emoji} +${act.xp} XP for ${act.label}. Side quests still count.`
...
function testN() {
  ...
  const opts = [
    { t:'⚡ TEST ALARM',  b:'Alarms active. You have zero excuses. None.' },
...
function resetDay() {
  if (!confirm("Reset ALL of today's task progress? Cannot be undone.")) return;
  ...
  renderAll(); addLog('Today reset.','ly'); toast('RESET','All tasks cleared.',true);
}

function resetAll() {
  if (!confirm('Delete ALL data? Wipes XP, streak, everything.')) return;
...
// ── Visibility change (wake from background) ──────────
...
(async function boot() {
  ...
  if ('Notification' in window) {
    if (Notification.permission === 'granted') {
      ...
      schedAll();
      checkMissedAlarms();
      addLog('Permission already granted. Alarms rescheduled.','lg');
  ...
  // PWA manifest
  try {
    const mf = { name:'LEVEL UP SYSTEM', short_name:'LEVEL UP', start_url:'./', display:'standalone', background_color:'#06060f', theme_color:'#06060f', icons:[{src:'./icon.svg',sizes:'192x192',type:'image/svg+xml'}] };
  ...
  addLog('System v3.0 boot complete.', 'lg');
})();
